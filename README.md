...
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 64 | 645.5 mm | 84.0% | 2.61% | 4 | 76.9% | 25.2% | 1.42 s | 927.6 MiB |
| **128** | **320.2 mm** | **89.7%** | **0.64%** | **4** | **81.5%** | **24.8%** | **2.72 s** | **1845.6 MiB** |
| 256 | 159.5 mm | 91.4% | 0.14% | 4 | 79.2% | 27.6% | 5.11 s | 3681.6 MiB |

`D=64` 无法表示小于约 0.65 m 的深度，近处标定板和桌面出现明显截断、散斑
和上限饱和。`D=128` 将有效率提高 5.7 个百分点，同时回投命中率提高
4.6 个百分点，是本场景更合理的默认搜索范围。

`D=256` 比 `D=128` 仅多 1.7 个百分点有效率，但时间从 2.72 s 增至
5.11 s、预计内存从 1.85 GiB 增至 3.68 GiB；回投命中率下降 2.4 个
...
|---:|---:|---:|---:|---:|---:|---:|
| 10/20 | **91.7%** | **0.62%** | 4 | 80.6% | 35.0% | 2.66 s |
| 20/40 | 89.7% | 0.64% | 4 | 81.5% | 24.8% | 2.72 s |
| **40/80** | 89.6% | 0.67% | **4** | **81.7%** | **19.9%** | 2.69 s |
...
固定 `D=128`、`preFilterCap=31`、`uniquenessRatio=15`、
`textureThreshold=20`，只改变 SAD 窗口：

![0724 StereoBM block size comparison](results/0724_imgs/comparison_bm_block_size.png)

| 块大小 | 有效率 | 上限饱和率 | 回投残差中位数 | 残差 ≤ 15 | 局部异常率 | CPU 时间 |
|---:|---:|---:|---:|---:|---:|---:|
| 5×5 | 45.2% | **0.29%** | 5 | 81.3% | 38.3% | **11.5 ms** |
| **11×11** | **47.3%** | 0.41% | 3 | 86.4% | 10.6% | **11.6 ms** |
| 21×21 | 46.1% | 0.42% | **2** | **91.2%** | **4.7%** | 20.5 ms |

5×5 保留更细的结构，但在高密度重复散斑上产生大量噪点。21×21 最平滑，
同时出现更明显的块状边缘、细结构丢失和空洞，并使时间增加到 20.5 ms。
11×11 的有效率最高，噪声、边缘和速度更均衡，验证结果支持继续使用原
Vitis BM 参考参数。
...
| CPU 实时速度优先 | **StereoBM，D=128，block=11** | 本机约 11.6 ms，但有效率仅 47.3% |
...
脚本会自动判定左右顺序、使用 YAML 完成校正、运行 8 组参数、生成 16 位毫米
深度图、固定色标图、三张横向对比图以及机器可读统计。详细的单组中间结果默认
